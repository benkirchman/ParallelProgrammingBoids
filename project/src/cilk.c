...
void display() {
	...
	//update();
	glBegin(GL_TRIANGLES);
	cilk_for(int i = 0; i< flockSize; i++) {
		look(i);
	}
	for(int i = 0; i< flockSize; i++) {
		move(i);
		glColor3f(((float)((flock[i]->xdir + 10)*10)+400)/600, ((float)((flock[i]->ydir + 10)*10)+400)/600, 200);
		glVertex2f(pixelToFloat(flock[i]->x), pixelToFloat(flock[i]->y));
	       	glVertex2f(pixelToFloat((flock[i]->x - flock[i]->xdir) - (flock[i]->ydir/2)*2), pixelToFloat((flock[i]->y - flock[i]->ydir) + (flock[i]->xdir/2)*2));
		glVertex2f(pixelToFloat((flock[i]->x - flock[i]->xdir) + (flock[i]->ydir/2)*2), pixelToFloat((flock[i]->y - flock[i]->ydir) - (flock[i]->xdir/2)*2));
	}
	glEnd();
	glutSwapBuffers();
...
void getVisibleBoids(int subject, int* avgx, int* avgy, int* avgxdir, int* avgydir) {
	int count = 0;
	cilk_for(int current = 0; current < flockSize; current++) {
...
void look(int i) {
	...
	//accumulate
	flock[i]->xdir +=  xdirTo + xdirAway + xdirAvg + xbounddir;
	flock[i]->ydir +=  ydirTo + ydirAway + ydirAvg + ybounddir;
	flock[i]->xdir /=2;	
 	flock[i]->ydir /=2;
...
void printAvgToFile() {
	fptr = fopen("/home/b/Documents/parallel/project/src/cilk_avg.csv","a");
...
void printFileHeader() {
	fptr = fopen("/home/b/Documents/parallel/project/src/cilk_avg.csv","w");
	fprintf(fptr,"f#,mspf\nboids,cilk");
	fclose(fptr);
}
