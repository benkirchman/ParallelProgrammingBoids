...
int main(int argc, char** argv) {
    ...
    char passmatch[8][9]; // buffer for the matched password
    ...
    int notfound=1;
    int sharedNotFound=1;
    char passmatchResult[9];
    while(sharedNotFound) {      
...
//	    printf("thread number %d\n", omp_get_thread_num());
    ...
    printf("Openmp found: %s in %dms\n",passmatchResult,msec);
